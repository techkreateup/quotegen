...
async function DELETE_handler(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    await prisma.entityActivity.deleteMany({ where: { entityType: "PurchaseOrder", entityId: id } }).catch(() => {});
    await prisma.entityNote.deleteMany({ where: { entityType: "PurchaseOrder", entityId: id } }).catch(() => {});
    await prisma.purchaseOrderLineItem.deleteMany({ where: { purchaseOrderId: id } });
